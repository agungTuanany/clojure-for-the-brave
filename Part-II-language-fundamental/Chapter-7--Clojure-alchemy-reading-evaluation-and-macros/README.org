#+TITLE: Clojure For The Brave and True Part II -- Chapter 7 Alchemy: Reading, Evaluation and Macros
#+DATE: 2025-10-08 Wed
...
philosopher's stone look like a mere trinket: the /macro/.
...
/(backwards (" backwards" " am" "I" str))/, even though it doesn't follow
...
order). Without /backwards/, the expression would fail harder than millenia of
alchemists ironically spending their entire lives pursuing an impossible means
of achieving immortality. With /backwards/, */you've created your own syntax/!*
...
the /reader/, the /evaluator/, and the /macro expander/. It's like the periodic
...
languages: it has a two-phase system where it /reads/ textual source code,
producing Clojure data structures. These data structures are then /evaluated/:
...
when Clojure reads the text /(+ 1 2)/, the result is a list data structure whose
first elements is a /+/ symbol, followed by the numbers /1/ and /2/. This data
structure is passed to Clojure's evaluator, which looks up the function
corresponding to /+/ and applies that function to /1/ and /2/.

Languages that have this relationship between source code, data, and evaluation
are called */homoiconic/*[fn:1]. (Incidentally, if you say /homoiconic/ in front
...
process, the compiler construct an /abstract syntax tree/ (AST), which is a data
structure that represents your program. You ca think of the AST as the input to
the /evaluator/, which you can think of as a function that traverses the tree to
...
#+caption: Figure 7-1: Evaluation in a non-lip programming language
#+attr_latex: scale=0.75
#+label: fig:label
[[file:figures-listing-and-images/Figure-7-1.png]]
...
#+caption: Figure 7-2: Lists can easily be treated as trees.
#+attr_latex: scale=0.75
#+label: fig:label
[[file:figures-listing-and-images/Figure-7-2.png]]

First, Clojure's /readers/ converts the text /(+ 1(* 6 7))/ into a nested list.
...
https://www.gigamonkeys.com/book/syntax-and-semantics.html

#+caption: Figure 7-3: Evaluation in Clojure
#+attr_latex: scale=0.75
#+label: fig:label
[[file:figures-listing-and-images/Figure-7-3.png]]
...
data structures directly to the Clojure evaluator with /eval/.
...
Clojure evaluated the list, it looked up the list that /addition-list/ refers
to; then it looked up the function corresponding to the /+/ symbol; and then it
called that function with /1/ and /2/ as arguments, returning /3/. The data
...
#+caption: Figure 7-4: The lists you evaluated
#+attr_latex: scale=0.75
#+label: fig:label
[[file:figures-listing-and-images/Figure-7-4.png]]
...
So Clojure /homoiconic/; it represents abstract syntax tree using lists, and you
...
with the reader and learn how a handy feature, the /reader macro/, lets you
write code succinctly[fn:2].
...
#+begin_src bash :results value verbatim :exports both eval: never-export
  user=> (str "To understand what recursion is, " " you must first understand
  recursion.")
#+end_src
...
of data structures is called /reader form/. In this example, the form represents
a list data structure that contains three more forms: the /str/ symbol and two
strings.

Once you type those characters into prompt and press /ENTER/, that text goes to
...
/read-string/ function. /read-string/ takes a string as an argument and
...
In the first examples, /read-string/ reads the string representation of a list
...
Whoa! This is not the one-to-one mapping that we're used to. Reading /#(+ 1 %)/
somehow resulted in a list consisting of the /fn*/ symbol, a vector containing a
...
form. They're designated by /macro characters/ like /'/ (the single quote), /#/,
and /@/. They're also completely different from the macros we'll get to later.
So as not to get the two confused, I'll always refer to reader macros using the
full term /reader macros/.
...
member is the symbol /quote/ and whose second member is the data structure that
followed the single quote. The /deref/ reader macro works similarly for the /@/
character:
...
For example, let's say you've types /(+ 1 2)/ in the REPL. Figure 7-5 shows a
diagram of the data structure that gets sent to the evaluator.

#+caption: Figure 7-5: The data structure for (+ 1 2)
#+attr_latex: scale=0.75
#+label: fig:label
[[file:figures-listing-and-images/Figure-7-3.png]]
...
function, the evaluator evaluates each of the operands. The operands /1/ and /2/
evaluate to themselves because they're not lists or symbols. Then the evaluator
calls the addition function with /1/ and /2/2 as the operands, and return the
result.
...
/def/, /let/, and functions, macros, data, and anything else you can use, and
evaluates them by /resolving/ them. to resolve a symbol, Clojure traverses[fn:3]
...
either a /value/ or /special form/--a built-in Clojure operator that provides
fundamental behavior.

In general, Clojure resolves a symbol by:

1. Looking up wether the symbol names a special form if it doesn't...
...
/if/, are always used in the context of an operation; they're always the first
element in a list:
...
In this case, /if/ is a special form and it's being used as an operator. If you
...
Next, let's evaluate some local bindings. A /local binding/ is any association
between a symbol and a value that wasn'tcreated by /def/. In the next example,
the symbol /x/ is bound to 5 using /let/. When the evaluator resolves /x/, it
resolves the /symbol x/ to the /value 5/:
...
Now if we create a namespace mapping of /x/ to /15/, Clojure resolve it
accordingly:
...
In the next example, /x/ is mapped to /15/, but we introdice a local binding of
/x/ to /5/ using /let/. so /x/ is resolved /5/.
...
function body. In this next example, /exclaim/ is mapped to a function. Within
the function body, the parameter name /exclamation/ is bound to the argument
...
When Clojure evaluates this code, it first evaluates the /map/ symbol, looking
up the corresponding function and applying it to its arguments. The symbol /map/
refers to the map function, but it shouldn't be confused with the function
itself. The /map/ symbol is still a data structure, the same way that the string
/"fried salad"/ is a data structure, but it's not the same as the function
itself:
...
Otherwise, it is evaluated as a /call/ to the first element in the list. The way
...
to the function as an argument. In this example, the /+/ symbol resolves to a
function:
...
rest of the elements in the list. The operands /1/ and /2/ both evaluate to
...
here: look up the /+/ symbol and evaluate each argument. To evaluate the list
/(+ 2 3)/, Clojure resolves the first member to the addition function and
...
Here, we ask clojure to evaluate a list beginning with the symbol /if/. That
/if/ symbol gets resolved to the /if/ special form, and Clojure calls that
...
/if/ you don't want each operand to be evaluated. You only want certain operands
to be evaluated, depending on whether the condition is true or false.

Another important special form is /quote/. You've seen lists represented like
this:
...
the first element in the a list. The /quote/ special form tells the evaluator,
...
/def/, /let/, /loop/, /fn/, /do/, and /recur/ are all special forms as well. You
...
beginning of each list. But there's more! You can also place a /macro/ at the
...
a function that makes Clojure read infix notation[fn:4] (such as /1 + 1/)
instead of its normal notation with the operator /(+ 1 1)/. This example is
/not/ a macro. Rather, it merely shows that you can write code using infix
...
However, /read-string/ returns a list, and you can use Clojure to reorganize
that list into something it /can/ successfully evaluate:
...
at [L-1] the macro /ignore-last-operand/ receives the list /(+ 1 2 10)/ as its
...
ignore it. By contrast, when you call a macro, the operands are /not/ evaluated.
...
evaluated, but the data structure returned by a macro /is/. The process of
determining the return value of macro is called /macro expansion/, and you can
use the function /macroexpand/ to see what data structure a macro returns before
...
#+label: fig:label
[[file:figures-listing-and-images/Figure-7-6.png]]

As you can see, both expansion result in the list /(+ 1 2)/. When this list is
evaluated, as in the previous example, the result is /3/.
...
reading and evaluation: the /macro expansion/ phase. Figure 7-6 sows how you can
visualize the entire evaluation process for /(infix (1 + 2))/.

And that's how macros fit into the evaluation process. But why would you want to
do this? The reason is that macros allows you to transform an arbitrary data
structure like /(1 + 2)/ into one that can Clojure can evaluate, /(+ 1 2)/. That
means /you can use Clojure to extend itself/ so you can write programs however
you please. In other words, macros enable /syntactic abstraction/. Syntactic
abstraction may sound a bit abstract (ha ha!). So let's explore that a little.

** Syntactic Abstraction and the -> Macro

Often, Clojure code consists of a bunch of nested function calls. For example, I
use the following function in one of my projects:
...
To understand the function body, you have to find the innermost form, in this
case /(clojure.java.io/resource path)/, and then work your way outward from
right to left to see how the result of each function gets passed to another
function. This right-to-left flow is opposite of what non-Lisp programmers are
used to. As you get used to writing in Clojure, this kind of code gets easier
and easier to understand. But if you want to translate Clojure code so you can
read it in a more familiar, left-to-right, to-to-bottom manner, you can use the
built-in /->/ macro, which is also known as the /threading/ or /stabby/ macro.
It lets you rewrite the preceding function like this:
...
You can read this as pipeline that goes from top to bottom instead of from inner
parentheses to outer parentheses. First, /path/ gets passed to /io/resource/,
then the result gets passed to /slurp/, and finally the result of that gets
passed to /read-string/.

These two ways of defining /read-resource/ are entirely equivalent. However, the
second one might be easier understand because we can approach it form top to
bottom, a direction we're used to. The /->/ also lets us omit parentheses, which
means there's less visual noise to contend with. This a /syntactic abstraction/
because it lets you write code in a syntax that's different from Clojure's
built-in syntax but is preferable for human consumption. Better than lead into
gold!!

* Summary

In this chapter, you learned about Clojure's evaluation process. First, the
reader transform text into Clojure data structures. Next, the macro expander
transforms those data structures with macros, converting your syntax into
syntactically valid data structures. Finally, those data structures get sent to
the evaluator. The evaluator processes data structures based on their type:
symbols are resolved to their referents; lists result in function, macro, or
special form calls; and everything else evaluates to itself.

The coolest thing about this process is that it allows you to use Clojure to
expand its own syntax. This process is made easier because Clojure is
homoiconic: its text represents data structures, and those data structures
represent abstract syntax trees, allowing you to more easily reason about how to
construct syntax-expanding macros.

With all these new concepts in your brainacles, you're now ready to blow stuff
up on purpose, just like I promised. The next chapter will teach you everything
you need to know about writing macros. Hold on to your sock or they're liable to
get knocked off!

* Exercises

These exercises focus on reading and evaluation. Chapter 8 has exercises for
writing macros.
...
2. Create an infix function that takes a list like /(1 + 3 * 4 - 5)/ and
   transforms it into the lists that Clojure need in order to correctly evaluate
   the expression using operator precedence rules.

* Footnotes
[fn:4] A mathematical expression format where operators are written between
operands they act on.

[fn:3] To visit all the nodes or elements in a data structure, such as a tree or
graph.

[fn:2] in a brief clearly expressed manner.

[fn:1] a property of a programming language where its code is represented as a
