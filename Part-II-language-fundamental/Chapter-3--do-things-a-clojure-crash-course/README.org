#+TITLE: Clojure For The Brave and True Part -- II Language Fundamental
#+DATE: 2025-08-13 Wed
...
#+RESULTS:
: {"string-key" #object[clojure.core$_PLUS_ 0x64a896b0 "clojure.core$_PLUS_@64a896b0"]}
...
#+begin_src clojure :result output :exports both eval: never-export
  (contains? #{[:first-name "Charlie"] [:last-name "McFishwich"]} [:first-name "Charlie"])
#+end_src
...
In the first example, the return value of /and/ is the first falsey value or the last truthy value. In this case, + is returned because it's the last truthy value, ans is then applied to the arguments 1 2 3, returning 6.
...
#+RESULTS:
: Execution error (ClassCastException) at user/eval1$fn (clojure-cmd-script-v3CMFT.clj:1).
...
: We're ignoring the rset of your choices. here they are in case you need to cry over Pigpen, Aquaman
...
n* pulling it all together

okay1, it's time to use your newfound knowledge for a noble purpose: smackinf
...
#+RESULTS:
: ; =>#'user/asym-hobbit-body-parts;; =>#'user/matching-part;; =>#'user/symmetrize-body-parts;; =>[{:name "head", :size 3}
: {:name "left-eye", :size 1}
